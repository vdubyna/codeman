...
def test_uv_run_index_build_lexical_supports_text_and_json_output(
    tmp_path: Path,
) -> None:
    ...
    assert "Built lexical index: 8 chunks" in text_result.stdout
    assert "Lexical engine: sqlite-fts5" in text_result.stdout
    assert payload["ok"] is True
    assert payload["data"]["build"]["lexical_engine"] == "sqlite-fts5"
    assert payload["data"]["diagnostics"]["chunks_indexed"] == 8
    assert "Building lexical index for snapshot" in text_result.stderr
    assert "Building lexical index for snapshot" in json_result.stderr


def test_uv_run_index_build_lexical_returns_stable_failure_when_chunks_are_missing(
    tmp_path: Path,
) -> None:
    project_root = Path(__file__).resolve().parents[2]
    ...
    payload = json.loads(build_lexical_result.stdout)

    assert register_result.returncode == 0, register_result.stderr
    assert snapshot_result.returncode == 0, snapshot_result.stderr
    assert extract_result.returncode == 0, extract_result.stderr
    assert build_lexical_result.returncode == 34

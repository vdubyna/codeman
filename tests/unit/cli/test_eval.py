...
def test_eval_benchmark_command_renders_text_summary_and_stderr_progress(tmp_path: Path) -> None:
    ...
    assert "Recall@K: 0.7500" in result.stdout
    assert (
        "Metrics Artifact Path: /tmp/.codeman/artifacts/benchmarks/run-123/metrics.json"
        in result.stdout
    )
...
def test_eval_benchmark_command_emits_clean_json_stdout(tmp_path: Path) -> None:
    ...
    assert payload["data"]["metrics"]["metrics"]["recall_at_k"] == 0.75
...
def test_eval_benchmark_command_returns_json_failure_envelope_for_metrics_error(
    tmp_path: Path,
) -> None:
    workspace = tmp_path / "workspace"
    workspace.mkdir()
    ...
    class StubRunBenchmarkUseCase:
        def execute(self, request: object, *, progress: object | None = None) -> object:
            raise BenchmarkMetricsError(
                "Benchmark metrics calculation failed for run: run-123",
                details={"run_id": "run-123", "reason": "disk full"},
            )
    ...
    assert payload["ok"] is False
    assert payload["error"]["code"] == "benchmark_metrics_calculation_failed"
    assert payload["error"]["details"] == {"run_id": "run-123", "reason": "disk full"}
    assert payload["meta"]["command"] == "eval.benchmark"
